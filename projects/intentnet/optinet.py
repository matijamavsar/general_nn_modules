...
class OptiNet(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        from ..networks import OptiNet
        self.model = OptiNet().to(args.dev)
    ...
    def shared_step(self, batch):
        ...
        outputs_demo = self.forward(inputs)
        ...
        loss = loss_demo
        return loss, outputs_demo, demo, seq_lengths, rgb, dep, traj

    def training_step(self, train_batch, batch_idx):
        loss, _, _, _, _, _, _ = self.shared_step(train_batch)
        return loss
    ...
    def validation_step(self, val_batch, batch_idx):
        loss, outputs_demo, demo, seq_lengths, rgb, dep, _ = self.shared_step(
            val_batch)
    ...
    def test_step(self, test_batch, batch_idx):
        _, outputs_demo, demo, seq_lengths, rgb, dep, traj = (
            self.shared_step(test_batch))
        acc = calculate_accuracy(outputs_demo, demo, seq_lengths)
        ...
        if ((self.current_epoch % self.args.test_interval == 0) and
            ...
            model_tester_pro(val_indices, outputs_demo, demo, self.writer,
                seq_lengths, self.current_epoch, rgb, dep, traj, 
                '/tmp/traj_fig.pkl')
            plt.savefig('pred_examples.pdf')
            breakpoint()
